...
class vcf:
    ...
    def run_snpeff(self,db):
        run_cmd(f"snpEff ann {db} {self.filename} | bcftools view -Oz -o {self.prefix}.ann.vcf.gz")
        return vcf(f"{self.prefix}.ann.vcf.gz")


    def load_ann(self,intragenic=False,upstream=False,downstream=False,noncoding=False,intronic=False,synonymous=False,splice=False):
        filter_out = []
        if intragenic==False:
        ...
        for l in cmd_out(f"bcftools query -f '%CHROM\\t%POS\\t%REF\\t%ALT\\t%ANN\\t[%AD]\\n' {self.filename}"):
            ...
            for a in annot:
                if len(annot[a])==0:
                    continue
                tmp_var = annot[a][0]
                tmp_var["alternate_consequnces"] = annot[a][1:]
